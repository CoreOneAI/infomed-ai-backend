...
/** ---------- CORS (whitelist your sites) ---------- */
const allowedOrigins = (process.env.ALLOWED_ORIGINS || "")
  .split(",")
  .map(s => s.trim())
  .filter(Boolean);

// sensible defaults for dev if not configured
...
app.post("/chat", async (req, res) => {
  try {
    ...
    const lang = (prefer.lang || "en").toLowerCase();            // "en" | "es"
    const providerReq = (prefer.provider || "auto").toLowerCase();// "openai"|"anthropic"|"gemini"|"auto"
    const system = buildSystemPrompt(lang, specialty);

    // pick the provider deterministically
    const run = async () => {
      if (providerReq === "openai")    return callOpenAI(system, message);
      if (providerReq === "anthropic") return callAnthropic(system, message);
      if (providerReq === "gemini")    return callGemini(system, message);

      // auto: prefer order OpenAI → Anthropic → Gemini
      if (process.env.OPENAI_API_KEY)    return callOpenAI(system, message);
      if (process.env.ANTHROPIC_API_KEY) return callAnthropic(system, message);
      if (process.env.GEMINI_API_KEY)    return callGemini(system, message);

      // fallback when no keys: echo
      return { text: `Echo: ${message}`, provider: "render-test" };
    };
...
function buildSystemPrompt(lang, specialty) {
  ...
  return [
    ...
    `Do not provide diagnosis; encourage clinician follow-up as needed.`,
    `Add brief safety caveats only when clinically appropriate.`
  ].join(" ");
}
...
async function callOpenAI(system, user) {
  if (!process.env.OPENAI_API_KEY) {
    return { text: `Echo: ${user}`, provider: "openai-missing-key" };
  }
...
async function callAnthropic(system, user) {
  if (!process.env.ANTHROPIC_API_KEY) {
    return { text: `Echo: ${user}`, provider: "anthropic-missing-key" };
  }
...
async function callGemini(system, user) {
  if (!process.env.GEMINI_API_KEY) {
    return { text: `Echo: ${user}`, provider: "gemini-missing-key" };
  }
...
// PORT logic: Render provides PORT; local defaults to 8080; dev can override.
